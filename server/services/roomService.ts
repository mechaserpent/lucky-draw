...
export async function joinRoom(
  ...
  // 檢查是否已在房間中
  const existingPlayer = [...room.players, ...room.spectators].find(p => p.id === playerId)
  if (existingPlayer) {
    // 更新連線狀態
  ...
  const participantId = role === 'player' 
...
export async function leaveRoom(roomId: string, playerId: string): Promise<Room | null> {
  const room = await loadRoomFromDb(roomId)
  if (!room) return null
...
// 玩家斷線處理
export async function handleDisconnect(roomId: string, playerId: string): Promise<Room | null> {
  const room = await loadRoomFromDb(roomId)
  if (!room) return null
  ...
  if (player.isHost) {
    const connectedPlayers = room.players.filter(p => p.id !== playerId && p.isConnected)
...
export async function handleReconnect(
  ...
  const player = [...room.players, ...room.spectators].find(p => p.id === newPlayerId)
...
// 將玩家轉為觀眾
export async function convertToSpectator(roomId: string, playerId: string): Promise<Room | null> {
  const room = await loadRoomFromDb(roomId)
  if (!room) return null
  if (!room.settings.allowSpectators) return null
...
export async function updateRoomSettings(
  ...
  await db.update(schema.rooms)
    .set({
      ...
      allowSpectators: settings.allowSpectators ?? room.settings.allowSpectators,
      drawMode: settings.drawMode ?? room.settings.drawMode,
      firstDrawerMode: settings.firstDrawerMode ?? room.settings.firstDrawerMode,
      firstDrawerId: settings.firstDrawerId ?? room.settings.firstDrawerId ?? null,
      updatedAt: new Date()
    })
    .where(eq(schema.rooms.id, roomId))
...
export async function setReady(roomId: string, playerId: string, ready: boolean): Promise<Room | null> {
  await db.update(schema.players)
    .set({ isReady: ready })
    .where(and(
      eq(schema.players.roomId, roomId),
...
export async function renamePlayer(roomId: string, playerId: string, newName: string): Promise<Room | null> {
  const room = await loadRoomFromDb(roomId)
  if (!room) return null
...
export async function startGame(roomId: string, seed?: number): Promise<Room | null> {
  const room = await loadRoomFromDb(roomId)
  if (!room) return null
  ...
  switch (room.settings.firstDrawerMode) {
    case 'host':
      firstDrawerId = room.players.find(p => p.isHost)?.participantId ?? playerIds[0]
      break
    case 'manual':
  ...
  // 清除舊的抽獎資料
  await db.delete(schema.drawSequences).where(eq(schema.drawSequences.roomId, roomId))
  await db.delete(schema.drawOrders).where(eq(schema.drawOrders.roomId, roomId))
  await db.delete(schema.drawResults).where(eq(schema.drawResults.roomId, roomId))
...
export async function restartGame(roomId: string): Promise<Room | null> {
  ...
  await db.update(schema.players)
    ...
    .where(and(
      eq(schema.players.roomId, roomId),
      eq(schema.players.isHost, false)
    ))
...
export async function getReconnectToken(roomId: string, playerId: string): Promise<string | null> {
  const playerData = await db.query.players.findFirst({
    where: and(
      eq(schema.players.roomId, roomId),
      eq(schema.players.playerId, playerId)
