...
export function startGame(roomId: string, seed?: number): Room | null {
  ...
  while (
    ...
    room.drawOrder.length < room.players.length
  ) {
...
export function performDraw(
  roomId: string,
  participantId?: number,
): { room: Room; result: DrawResult } | null {
  const room = rooms.get(roomId);
  if (!room) return null;
  ...
  const drawer = room.players.find((p) => p.participantId === result.drawerId);
  const giftOwner = room.players.find(
    (p) => p.participantId === result.giftOwnerId,
  );

  const enrichedResult = {
    ...result,
    isRevealed: false,
    // set performedAt now for consistency with DB (will be updated on reveal)
    performedAt: Date.now(),
    drawerName: drawer?.name ?? null,
    giftOwnerName: giftOwner?.name ?? null,
  };

  room.results.push(enrichedResult);

  return { room, result: enrichedResult };
}

// reveal a result (idempotent)
export function revealResult(
  ...
  const r = room.results.find((x) => x.order === order);
  if (!r) return null;

  if (!r.isRevealed) {
    r.isRevealed = true;
    r.performedAt = Date.now();
  }

  return { room, result: r };
}
...
// 更新房間人數上限
export function updateRoomMaxPlayers(
  roomId: string,
  hostId: string,
